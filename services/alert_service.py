...
def generar_alertas_mora(db: Session, studio_id: int) -> int:
    ...
    try:
        ...
        # Cobro no tiene cliente_id; se llega al cliente a través de Abono
        ...
        for cobro in cobros_vencidos:
            ...
            cliente_id = abono.cliente_id if abono else None
            cliente = db.query(Cliente).filter(Cliente.id == cliente_id).first() if cliente_id else None
            nombre = cliente.nombre if cliente else f"Cobro #{cobro.id}"
