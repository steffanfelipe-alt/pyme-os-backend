    ...
    try:
        ...
        for studio in studios:
            ...
            if not dueno or not dueno.email:
                logger.warning("Job resumen semanal — estudio %d sin dueño con email", studio_id)
                continue
            ...
            try:
                ...
                db.commit()
                logger.info("Job resumen semanal — email enviado a %s (estudio %d)", dueno.email, studio_id)
            except Exception as e:
                logger.error("Job resumen semanal — error enviando email a estudio %d: %s", studio_id, e)
