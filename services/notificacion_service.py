...
def job_resumen_semanal_email() -> None:
    ...
    try:
        ...
        studios = db.query(Studio).all()
        for studio in studios:
            # Buscar dueño del estudio actual
            ...
            if not dueno or not dueno.email:
                logger.warning(
                    "Job resumen semanal — estudio %d sin dueño con email configurado", studio.id
                )
                continue

            # Vencimientos de la semana para este estudio
            vencimientos_semana = (
                db.query(Vencimiento, Cliente)
                .join(Cliente, Vencimiento.cliente_id == Cliente.id)
                .filter(
            ...
            # Alertas activas para este estudio
            ...
            try:
                ...
                logger.error("Job resumen semanal — error enviando email para estudio %d: %s", studio.id, e)
