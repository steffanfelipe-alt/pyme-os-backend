...
def job_resumen_semanal_email() -> None:
    ...
    try:
        ...
        studios = db.query(Studio).all()
        if not studios:
            logger.warning("Job resumen semanal — no hay estudios configurados")
            return
        ...
        for studio in studios:
            ...
            if not dueno or not dueno.email:
                logger.warning("Job resumen semanal — studio %d sin dueño con email", studio_id)
                continue
            ...
            try:
                ...
                logger.error("Job resumen semanal — error enviando email a studio %d: %s", studio_id, e)
