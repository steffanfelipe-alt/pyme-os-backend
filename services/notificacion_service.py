...
def job_resumen_semanal_email() -> None:
    ...
    try:
        ...
        for studio in studios:
            # Buscar dueño de este estudio específico
            ...
            if not dueno or not dueno.email:
                logger.info("Job resumen semanal — estudio %d sin dueño con email, omitido", studio.id)
                continue

            # Vencimientos de la semana filtrados por studio
            vencimientos_semana = (
                db.query(Vencimiento, Cliente)
                .join(Cliente, Vencimiento.cliente_id == Cliente.id)
                .filter(
            ...
            try:
                ...
                db.commit()
                emails_enviados += 1
                logger.info("Job resumen semanal — email enviado a %s (estudio %d)", dueno.email, studio.id)
            except Exception as e:
                logger.error("Job resumen semanal — error enviando email a %s: %s", dueno.email, e)
