...
def job_resumen_semanal_email() -> None:
    ...
    try:
        ...
        studios = db.query(Studio).all()
        for studio in studios:
            _enviar_resumen_semanal_studio(db, studio.id, hoy, fin_semana, RolEmpleado, AlertaVencimiento)
...
def _enviar_resumen_semanal_studio(db: Session, studio_id: int, hoy: date, fin_semana: date, RolEmpleado, AlertaVencimiento) -> None:
    ...
    if not dueno or not dueno.email:
        logger.warning("Job resumen semanal — estudio %d sin dueño con email configurado", studio_id)
        return
    ...
    try:
        ...
        db.commit()
        logger.info("Job resumen semanal — email enviado a estudio %d → %s", studio_id, dueno.email)
    except Exception as e:
        logger.error("Job resumen semanal — error enviando email estudio %d: %s", studio_id, e)
