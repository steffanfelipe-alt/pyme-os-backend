...
def job_resumen_semanal_email() -> None:
    ...
    try:
        ...
        for studio in studios:
            ...
            # Buscar dueño del estudio filtrado por studio_id
            ...
            if not dueno or not dueno.email:
                logger.warning("Job resumen semanal — no hay dueño con email para studio %d", studio_id)
                continue

            # Vencimientos de la semana filtrados por studio_id
            vencimientos_semana = (
                db.query(Vencimiento, Cliente)
                .join(Cliente, Vencimiento.cliente_id == Cliente.id)
                .filter(
            ...
            # Alertas activas filtradas por studio_id
            ...
            try:
                ...
                db.commit()
                logger.info("Job resumen semanal — email enviado a %s (studio %d)", dueno.email, studio_id)
                emails_enviados += 1
            except Exception as e:
                logger.error("Job resumen semanal — error enviando email para studio %d: %s", studio_id, e)
